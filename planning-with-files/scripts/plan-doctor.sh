...
#
# Diagnostic only. Writes only inject-plan.sh's private SHA and progress caches.
# Always exits 0.
...
if [ -f "${INJ}" ]; then
    ...
    if [ -z "${OUT}" ]; then
        ...
        case "${OUT}" in
            *'PLAN TAMPERED'*)
                warn "injection: plan is attested but the hash mismatches — run scripts/attest-plan.sh to re-approve the current plan"
                ;;
    ...
    warn "inject-plan.sh not found next to plan-doctor — reinstall the complete skill package"
fi
...
if [ -n "${ATT}" ]; then
    ...
    info "attestation: none (opt-in in legacy mode; default-on in v3 modes; run scripts/attest-plan.sh after approving the plan)"
fi
...
info "hook reminder: hooks.json expects adapters under .codex/hooks or the user-level equivalent; see the Hook Installation section in SKILL.md"
