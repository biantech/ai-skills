...
def skill_script_path(script_name: str) -> Path | None:
    """Locate a planning-with-files helper in source and installed layouts."""
    candidates: list[Path] = []
    override = os.environ.get("PWF_SCRIPT_DIR", "")
    if override:
        candidates.append(Path(override) / script_name)
    candidates.extend([
        HOOK_DIR.parent / "scripts" / script_name,
        HOOK_DIR.parent / "skills" / "planning-with-files" / "scripts" / script_name,
        Path.home() / ".codex" / "skills" / "planning-with-files" / "scripts" / script_name,
    ])
    ...
    *args: str,
    ...
    if os.name == "nt":
        ...
        if sh_path is None:
            ...
            # hook must never surface an error. Git for Windows provides the
            # POSIX shell required by these helpers.
            return "", ""
        sh_cmd = sh_path
        if extra_dirs:
...
def run_shell_script(
    ...
    *args: str,
    ...
    env = {"PWF_SESSION_ID": session_id} if session_id else None
    return _run_script(
        HOOK_DIR / script_name,
        cwd,
        *args,
...
def run_skill_script(
    ...
    *args: str,
    ...
    script_path = skill_script_path(script_name)
