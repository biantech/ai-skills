# Graphify Workflow Reference

This reference contains the detailed execution flow for the `$graphify` Codex
Skill. Follow the runtime and safety rules in `../SKILL.md` first. In every
snippet that imports `graphify`, set `PYTHONPATH` to include
`$GRAPHIFY_SKILL_DIR/scripts`.

Before running the snippets, resolve the Skill directory and export the bundled
module path in the shell that executes them:

```bash
export GRAPHIFY_SKILL_DIR="/absolute/path/to/graphify"
export PYTHONPATH="$GRAPHIFY_SKILL_DIR/scripts${PYTHONPATH:+:$PYTHONPATH}"
```
...
Three things it adds beyond reading the corpus directly:
...
This step has two parts: **structural extraction** (deterministic) and **semantic extraction** (Codex sub-agents, costs tokens).
...
Use collaboration sub-agents for semantic extraction. Dispatch independent
chunks concurrently, subject to the available concurrency limit.
...
python3 -c "
import json
from graphify.cache import check_semantic_cache_full
from pathlib import Path

detect = json.loads(Path('.graphify_detect.json').read_text())
all_files = [f for files in detect['files'].values() for f in files]

cached_nodes, cached_edges, cached_hyperedges, uncached = check_semantic_cache_full(all_files)
...
Dispatch one sub-agent per chunk before waiting for results.

Concrete example for 3 chunks:
```
[sub-agent 1: files 1-15]
[sub-agent 2: files 16-30]
[sub-agent 3: files 31-45]
```
Wait for all dispatched chunks after starting them.
...
This starts a blocking stdio MCP server that exposes tools: `query_graph`,
`get_node`, `get_neighbors`, `get_community`, `god_nodes`,
`graph_stats`, and `shortest_path`. Start it in a separate persistent
process and use it from an MCP-compatible client.
...
```bash
python3 -c "
from graphify.update import merge_incremental
...
The helper persists the merged extraction and preserves old and new
hyperedges. Then run Steps 4-8 on `.graphify_extract.json` as normal.
...
Do not modify project instruction files unless the user asks. When requested,
add a short Graphify section to the project's existing Codex instruction file
