...
def check_semantic_cache(
    ...
    Uncached files need model extraction; cached files are merged directly.
    """
...
def check_semantic_cache_full(
    files: list[str],
    root: Path = Path("."),
) -> tuple[list[dict], list[dict], list[dict], list[str]]:
    """Return cached nodes, edges, hyperedges, and uncached file paths."""
    cached_nodes: list[dict] = []
    cached_edges: list[dict] = []
...
def save_semantic_cache(
    ...
    for h in hyperedges or []:
        ...
        if src:
            by_file[src].setdefault("hyperedges", []).append(h)
