...
In this repo, create and use `output/playwright/<label>/` to keep artifacts contained:
...
- If the wrapper fails while fetching the package, check `node`, `npm`, registry, and proxy access; `npx` being present alone is not sufficient.
- Never commit files containing passwords, tokens, cookies, authorization headers, personal data, or raw network responses. Prefer test accounts and environment variables.
