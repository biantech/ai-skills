...
For a checkout copy, use the absolute path to `scripts/playwright_cli.sh`.

To pin the package used by the wrapper:
...
Session data is in-memory by default. Do not save auth state, cookies, traces, or network logs containing secrets into the repository.
